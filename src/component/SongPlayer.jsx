...
/*function SongPlayer() {
  ...
  useEffect(() => {
    if (currentSong && isPlaying) audioRef.current.play();
  ...
  const toggleRepeat = () => setIsReplay(!isReplay);
  ...
  return (
    <div className="music-player">
      <div className="controls">
        ...
        {isReplay ? (
          <TbRepeat onClick={toggleRepeat} />
        ) : (
          <TbRepeatOff onClick={toggleRepeat} />
        )}
        <span>{formatTime(currentTime)}</span>
      </div>

      <input type="range" min="0" max="100" value={progress} onChange={seeek} />
      ...
      {volume === 0 ? <FaVolumeOff /> : <FaVolumeHigh />}
    </div>
  );
}
